...
export default function SafeDestinationsSheet({
  ...
  return (
    <div className="flex flex-col gap-3">
      <h2 className="text-sm font-bold uppercase tracking-wide text-ink">Safe destinations</h2>
      {candidates.length === 0 && (
      ...
      {candidates.map((c, i) => (
        <button
          ...
          className={`flex w-full items-center gap-3 rounded-xl border p-3 text-left transition-colors hover:bg-surface-overlay ${
            i === 0 ? 'border-safety bg-safety/5' : 'border-line'
          }`}
        >
          <span aria-hidden className="text-xl">{CATEGORIES[c.location.category]?.glyph}</span>
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-semibold text-ink">
              {i === 0 && <span className="mr-1.5 rounded bg-safety px-1.5 py-0.5 text-[10px] font-bold text-white">BEST</span>}
              {c.location.name}
            </span>
